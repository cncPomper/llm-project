...
- **A re-ingest is only idempotent because of `purge_document()`.** Determinism alone is not enough: the Postgres inserts are `ON CONFLICT (id) DO NOTHING`, so an existing row keeps its *old* text while Qdrant's `upsert` replaces the vector — the two stores then describe different chunks under one id. Anything that changes how many chunks an episode yields also strands the previous run's surplus tail, since ids only overwrite index-for-index. `purge_document()` clears both stores for the episode first, and runs whenever `force`/`REINGEST=1` is set.
- **After changing any chunking constant** (`FLAT_CHUNK_TOKENS`, `PARENT_WINDOW_TOKENS`, …) or the ad filter, existing episodes are silently skipped. Set `REINGEST=1` to rebuild them.
- **Sponsor reads are stripped at ingestion** by `ingestion/ad_filter.py`, on raw segments before chunking — the one point feeding both indexes, so they cannot disagree about the corpus. `STRIP_ADS=0` disables it. `python -m ingestion.ad_filter` is a dry run that prints what each episode would lose. Detection is conservative by design: dropping an ad is a small win, dropping real content is a silent loss. Two markers were tried and rejected against this corpus — `terms and conditions` (8 false positives on a genuine discussion of scam checkout terms) and bare `sponsor` (matches disclosures and outros). Spans are held open by *brand repetition*, not marker density, because an ad's body is ordinary prose.
